...
def _mirror_face(base: str, fill: str, rotation: int) -> Tuple[str, int]:
    ...
    if base in _LIMB_BASES:
        new_rotation = _mirror_limb_rotation(rotation)
    elif base in _OTHER_16_ROTATION_BASES:
        new_rotation = (rotation + 8) % 16
    elif base in _OTHER_XOR_4_BASES:
        new_rotation = rotation ^ 4
    elif base in _OTHER_XOR_2_BASES:
        new_rotation = rotation ^ 2
    elif base in _OTHER_XOR_1_BASES:
        new_rotation = rotation ^ 1
    elif base in _FACE_WITH_0_4_SWAP_BASES:
        new_rotation = _FACE_WITH_0_4_SWAP_ROTATION.get(rotation, rotation)
    else:
